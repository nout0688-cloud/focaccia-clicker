// Симуляция античита v5 (точный порт формул из App.tsx) — проверка разделения
...
function behaviourScore(state, taps) {
  let b = 0;
  if (state.syntheticRecent >= 8) b += 60;
...
// ==== Генераторы потоков тапов (t, x, y) ====
const r = (a, b) => a + Math.random() * (b - a);
const gauss = () => (Math.random() + Math.random() + Math.random() + Math.random() - 2) / 1.5;

function humanStream(n, base) {
  const out = [];
  let t = 0, x = 105, y = 210, rate = base;
  let anchorX = 0, anchorY = 0; // палец медленно плавает вокруг якоря
  for (let i = 0; i < n; i++) {
    ...
    t += Math.max(45, rate + gauss() * rate * 0.15 + pause);
    // палец: якорь дрейфует медленно + разброс удара 6-10px + редкие прыжки 20-30px
...
function userBotStream(n, interval, jitter, drift) {
  const out = [];
  let t = 0, x = 105, y = 210, phase = Math.random() * 6;
  for (let i = 0; i < n; i++) {
    t += Math.max(20, interval + r(-jitter, jitter));
    phase += 0.25;
...
function perfectBotStream(n) {
  ...
  for (let i = 0; i < n; i++) {
    t += 100;
    x = 105 + Math.sin(t / 250) * 3;
    y = 210 + Math.cos(t / 250) * 3;
    out.push({ t, x, y });
  }
  return out;
}

// ==== Прогон trigger-логики по длинному потоку ====
function runTrigger(stream, platform, label, maxTaps = 3000) {
  ...
  for (let i = 10; i < taps.length; i++) {
    const buf = taps.slice(Math.max(0, i - 360), i);
    ...
    const H = humanScore(t40, platform);
    ...
    const evidence = clamp(0.50 * R + 0.25 * C + 0.25 * B - 0.35 * H + boost, 0, 100);
    ...
    const strong = recent.filter((v) => v >= 12).length;
    const veryStrong = recent.filter((v) => v >= 20).length;
    const indep = (R >= 60 ? 1 : 0) + (C >= 45 ? 1 : 0) + (B >= 45 ? 1 : 0);
    if (suspicion >= 16 && strong >= 4 && veryStrong >= 2 && indep >= 1) {
  ...
  console.log(`${label}: ${triggeredAt >= 0 ? `TRIGGER через ${(triggeredAt / 1000).toFixed(0)}с / ${Math.round(taps.findIndex((t) => t.t - t0 >= triggeredAt) + 1)} тапов` : 'нет триггера'}  [${dbgAvg}]`);
}

const state = { syntheticRecent: 0, bLongSession: false, bFastStreak: false };

console.log('=== v5 spec-формулы: базовая проверка ===');
runTrigger(humanStream(3000, 330), 'ios', 'человек 3/с (10 мин)');
runTrigger(humanStream(3000, 145), 'ios', 'человек 7/с (10 мин)');
